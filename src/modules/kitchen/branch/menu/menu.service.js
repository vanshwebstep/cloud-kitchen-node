...
export const createMenuItem = async (data) => {
    const { kitchenId, branchId, name, description, price, category, subCategory, ingredients = [] } = data;
    DebugHelper.debug('[MenuItem Service] Saving new menuItem...');
    DebugHelper.debug('[MenuItem Service] Data:', JSON.stringify(data, null, 2));
    try {
        // ===============================================
        // 🧠 TRANSACTION START
        // ===============================================
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (category?.id) {
                ...
                const createdCategory = await tx.menuCategory.upsert({
                    ...
                    update: {},
                    create: {
                        name: stringHelper.toTitleCase(category.name.trim()),
            ...
            if (subCategory?.id) {
                ...
                // ===============================================
                if (!categoryId) {
                    throw new Error("Category is required for sub category");
                }
                const normalizedSubCategoryName = stringHelper.toTitleCase(subCategory.name.trim());
                // ===============================================
            ...
            if (ingredients.length > 0) {
                const inventoryItems = await tx.branchIngredientInventory.findMany({
                    where: {
                        ...
                        ingredientId: {
                            in: ingredients.map(item => BigInt(item.id))
                        }
                    },
                    select: {
                        id: true,
                ...
                const inventoryMap = new Map(inventoryItems.map(item => [
                ...
                await tx.menuItemIngredient.createMany({
                    data: ingredients.map(ingredient => ({
                        ...
                        inventoryItemId: inventoryMap.get(ingredient.id),
        ...
        DebugHelper.debugError('[MenuItem Service] Error:', error);
        return {
            status: false,
...
export const getMenuItemes = async (params) => {
    try {
        ...
        DebugHelper.debug(`[MenuItem Service] Fetching menuItems | Page: ${page}`);
        // ===============================================
        // 🔍 BUILD WHERE
        // ===============================================
        const where = {
        ...
        DebugHelper.debugError(`[MenuItem Service] getMenuItemes failed: ${error.message}`);
        return {
            status: false,
...
export const getMenuItemById = async (id) => {
    try {
        DebugHelper.debug(`[MenuItem Service] Fetching menuItem: ${id}`);
        const response = await menuItemRepo.findUnique({
            where: { id },
            include: {
                category: true,
                subCategory: true,
        ...
        DebugHelper.debugError(`[MenuItem Service] getMenuItemById failed: ${error.message}`);
        return {
            status: false,
...
export const updateMenuItem = async (id, data) => {
    try {
        DebugHelper.debug(`[MenuItem Service] Updating menuItem: ${id}`);
        const existing = await menuItemRepo.findUnique({
            where: { id }
        });
        if (!existing?.data) {
            return {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (data.category?.id) {
                ...
                const category = await tx.menuCategory.upsert({
                    ...
                    update: {},
                    create: {
                        name: stringHelper.toTitleCase(data.category.name.trim()),
                        status: Status.PENDING
                    }
                });
                categoryId =
            ...
            if (data.subCategory?.id) {
                ...
                const subCategory = await tx.menuCategory.upsert({
                    ...
                    update: {},
                    create: {
                        name: stringHelper.toTitleCase(data.subCategory.name.trim()),
                        category: {
                            connect: {
                                id: categoryId
                            }
            ...
            if (Array.isArray(data.ingredients)) {
                ...
                if (data.ingredients.length > 0) {
                    const inventoryItems = await tx.branchIngredientInventory.findMany({
                        where: {
                            ingredientId: {
                                in: data.ingredients.map((item) => BigInt(item.id))
                            }
                        },
                        select: {
                            id: true,
        ...
        DebugHelper.debugError(`[MenuItem Service] updateMenuItem failed: ${error.message}`);
        return {
            status: false,
...
export const deleteMenuItem = async (id) => {
    try {
        DebugHelper.debug(`[MenuItem Service] Deleting menuItem: ${id}`);
        const existing = await menuItemRepo.findUnique({
            where: { id }
        });
        if (!existing?.data) {
        ...
        const response = await menuItemRepo.update(Number(id), {
        ...
        DebugHelper.debugError(`[MenuItem Service] deleteMenuItem failed: ${error.message}`);
        return {
            status: false,
...
export const updateMenuItemStatus = async (id, status) => {
    try {
        DebugHelper.debug(`[MenuItem Service] Updating status: ${id} -> ${status}`);
        const existing = await menuItemRepo.findUnique({
            where: {
                id: Number(id)
            }
        ...
        const response = await menuItemRepo.update(Number(id), { status });
        return {
            status: true,
        ...
        DebugHelper.debugError(`[MenuItem Service] updateMenuItemStatus failed: ${error.message}`);
        return {
            status: false,
            message: 'Something went wrong'
        };
