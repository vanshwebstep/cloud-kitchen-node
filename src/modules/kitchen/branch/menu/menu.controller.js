...
export const createMenuItem = async (req, res) => {
    ...
    try {
        ...
        const { name, description, price, category, subCategory, ingredients } = request.body;
        const errors = {};
        // ===============================================
        ...
        if (subCategory?.id) {
            ...
            if (!subCategoryResponse.status ||
                ...
                // ❌ validate parent category relation
                    ...
                    if (Number(subCategoryResponse.data.parentId)
                        !== Number(category.id)) {
                        errors.subCategory =
        ...
        if (ingredients?.length) {
            ...
            const ingredientBigIntIds = ingredientIds.map((id) => BigInt(id));
            ...
            const foundIds = new Set(inventoryResponse.data.map((item) => Number(item.ingredientId)));
            const invalidIngredients = ingredientIds.filter((id) => !foundIds.has(id));
        ...
        // 🚀 CREATE MENU ITEM
        // ===============================================
        debugHelper.debug('[MenuItem Controller] Calling MenuItemService.createMenuItem...');
...
// 📄 GET ALL MENUES
// =====================================================
export const getMenuItemes = async (req, res) => {
...
export const updateMenuItem = async (req, res) => {
    ...
    try {
        ...
        const { name, category, subCategory, ingredients } = req.body;
        const errors = {};
        // ===============================================
        ...
        // 🔒 CHECK OWNERSHIP
        // ===============================================
        if (Number(existingMenuItem.data.kitchenId)
            !== Number(kitchenId)) {
        ...
        if (ingredients?.length) {
            ...
            const inventoryResponse = await branchIngredientInventoryRepo.findMany({
                where: {
                    kitchenId: BigInt(kitchenId),
                    ingredientId: {
                        in: ingredientIds.map((id) => BigInt(id))
                    }
                },
                select: {
                    ingredientId: true
            ...
            const foundIds = new Set(inventoryResponse.data.map((item) => Number(item.ingredientId)));
            const invalidIngredients = ingredientIds.filter((ingredientId) => !foundIds.has(ingredientId));
            if (invalidIngredients.length > 0) {
                errors.ingredients =
                    `Invalid ingredient ids: ${invalidIngredients.join(', ')}`;
            }
        ...
        debugHelper.debugWarn('updateData', JSON.stringify(updateData, null, 2));
        // ===============================================
        // 🚀 UPDATE MENU ITEM
        // ===============================================
