...
export const createIngredient = async (data) => {
    ...
    DebugHelper.debug('[Ingredient Service] Data:', JSON.stringify(data, null, 2));
    try {
        // ===============================================
        // 🧠 TRANSACTION START
        // ===============================================
        const result = await prisma.$transaction(async (tx) => {
            ...
            // 🛡️ REMOVE DUPLICATES
            // ===============================================
            const uniqueIngredients = Array.from(new Map(newIngredients.map(i => [i.name, i])).values());
            // ===============================================
            ...
            if (uniqueIngredients.length > 0) {
                const createdIngredients = await Promise.all(uniqueIngredients.map(item => tx.ingredient.upsert({
                    where: {
                        name: item.name
                    },
                    update: {},
                ...
                createdIngredientIds = createdIngredients.map(i => Number(i.id));
            }
            // ===============================================
            // 3️⃣ MERGE ALL IDS
            // ===============================================
            ...
            if (finalIngredientIds.length > 0) {
                const inventoryPayload = finalIngredientIds.map(id => {
                    const ingredientData = ingredients.find(i => Number(i.id) === id);
                    return {
                        kitchenId,
                        branchId,
                        ingredientId: id,
        ...
        DebugHelper.debug('[Ingredient Service] Ingredients saved successfully', JSON.stringify(stringHelper.convertBigInt(result, "number"), null, 2));
        return {
            status: true,
...
export const getIngredients = async (params) => {
    try {
        ...
        debugHelper.debug(`[Ingredient Service] Fetching ingredients | Page: ${page}`, filters);
        const skip = (page - 1) * limit;
        DebugHelper.debug(`[Ingredient Service] Fetching Ingredients | Page: ${page}`);
        // ===============================================
        // 🔍 BUILD WHERE FILTER
        // ===============================================
        const where = {
