...
export const createStock = async (data) => {
    const { kitchenId, branchId, stocks } = data;
    try {
        DebugHelper.debug('[Stock Service] Create Stock Payload', JSON.stringify(data, null, 2));
        // ===============================================
        // 🧠 TRANSACTION START
        // ===============================================
        const result = await prisma.$transaction(async (tx) => {
            ...
            // ✅ INGREDIENT IDS
            // ===========================================
            const ingredientIds = stocks.map(item => BigInt(item.id));
            // ===========================================
            ...
            const payload = stocks
                .map(item => {
                const inventoryItem = inventoryItems.find(inv => Number(inv.ingredientId) === Number(item.id));
                // ❌ skip if not mapped
                if (!inventoryItem) {
                    return null;
                }
            ...
            // ===========================================
            if (payload.length === 0) {
                throw new Error('No valid mapped ingredients found for this branch');
            }
            // ===========================================
            // ✅ INSERT STOCKS
            // ===========================================
        ...
        DebugHelper.debugError('[Stock Service] createStock Error', error);
        return {
            status: false,
...
export const getStocks = async (params) => {
    try {
        ...
        DebugHelper.debug(`[Stock Service] Fetching Stocks | Page: ${page}`, filters);
        // ===============================================
        // 🔍 BUILD WHERE FILTER
        // ===============================================
        const where = {
        ...
        DebugHelper.debugError(`[Stock Service] getStocks failed`, error);
        return {
            status: false,
