 ...
 */
export const loginKitchen = async (data) => {
    try {
 ...
 */
export const getUserByUsername = async (username, type) => {
    try {
        if (!username || !type) {
            return {
                status: false,
        ...
        const whereCondition = type === "username"
            ? { username }
            : type === "email"
                ? { email: username }
                : { phone: username };
 ...
 */
export const checkUserAvailability = async (identifier) => {
    try {
        if (!identifier) {
            return {
                available: false,
...
export const createKitchen = async (data) => {
    ...
    if (existingUsers.data && existingUsers.data.length > 0) {
        const usedFields = new Set([
            ...existingUsers.data
                .map((user) => user.phone === phone ? 'Phone' : null)
                .filter(Boolean),
            ...existingUsers.data
                .map((user) => user.email === email ? 'Email' : null)
                .filter(Boolean),
        ]);
